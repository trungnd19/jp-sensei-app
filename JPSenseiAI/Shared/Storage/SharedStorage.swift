...
final class SharedStorage: ObservableObject {
    ...
    func getModel() -> String {
        let saved = defaults.string(forKey: Constants.StorageKeys.model) ?? getProvider().defaultModel
        // Fallback if saved model was deprecated/shut down
        let provider = getProvider()
        if !provider.availableModels.contains(saved) {
